...
const VideoConfRoom = () => {
  ...
  const [itemId, setItemId] = useLocalStorage(false);
  ...
  useEffect(() => {
    ...
    // console.log("INFO", pathurl.current.pathname.length, storeUser);
    ...
    if (query && query.id && pathurl.current.pathname.length == 1) {
      ...
      //TODO
      setLoginUser("/");
    } else if (pathurl.current.pathname == "/podcast" && query && query.id) {
      // console.log("pathurl", pathurl.current.pathname);
      setItemId(query.id);
      setSharedVideo(true);
    ...
    navigator.mediaDevices
      ...
      .then(stream => {
        ...
        socketRef.current.on("video muted", payload => {
          // console.log("video muted", payload, stream);
          stream
            .getVideoTracks()
            .forEach(track => (track.enabled = payload.videoStatus));
  ...
  function toggleVideo(info) {
    console.log("xxxx", socketRef.current);
    if (videoStarted == false && socketRef.current) {
      // alert(222);
      startVideoSession();
      ...
      alert("Please make sure you are logged in to get started!");
    }
  }
  ...
  return (
    <Fragment>
      ...
      ) : loginUser && loginUser == "schedule" && cookie.load("vare") ? (
        <ScheduleModal setLoginUser={setLoginUser} loginUser={loginUser} />
      ) : loginUser && loginUser == "podcast" ? (
        <PodcastModal
          saveMessageLikes={saveMessageLikes}
        ...
        >
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      ...
                      {` ${
                        !meetingId
                          ? "There is no meeting tied to this URL, please contact your meeting admin for more details"
                          : !cookie.load("vare")
            ...
            <Colxx lg={6} sm={12}>
              ...
              {meetingId && storeUser && storeUser.name ? (
                <LiveChats
                  replyId={replyId}
                  msg={msg}
                  saveChatList={chat => {
