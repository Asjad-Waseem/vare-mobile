...
const Video = props => {
  const {displayVideo, setDisplayVideo, name, audioStatus} = props;
  const ref = useRef();
  ...
  useEffect(() => {
    // console.log("displayVideoInner", displayVideo);
  }, [displayVideo]);
...
const VideoConfRoom = () => {
  ...
  function startVideoSession() {
    ...
    navigator.mediaDevices
      .getUserMedia({
        video: true, //videoStatus.current,
        audio: true
      })
      .then(stream => {
        ...
        socketRef.current.on("user joined", payload => {
          // console.log("remove signal", peers, payload);
        ...
        socketRef.current.on("audio muted", payload => {
          const peerObj =
            audioPeerSatus && audioPeerSatus.length > 0
              ? audioPeerSatus.find(p => p.peerID === payload.id)
              : "";
          if (peerObj) {
  ...
  function muteVideo() {
    if (videoStarted == false) {
      ...
      navigator.mediaDevices
        ...
        .then(stream => {
          // console.log("videoStatus", stream);
          userVideo.current.srcObject = stream;
        });
    }
  }
