...
const Home = ({
  ...
  const [navItems, setNavItems] = useState([
      { id: 1 , idnm : "VareHall", navheading: "VareHall" },
      // { id: 2 , idnm : "Comments", navheading: "Comments" },
      { id: 3 , idnm : "https://play.google.com/store/apps/details?id=com.vote.keyVoteApp", navheading: "Android" },
      { id: 4 , idnm : "https://apps.apple.com/app/id1503031565", navheading: "IOS" }
  ])

  const [activeFooterNav, setActiveFooterNav] = useState('Home');

  const [navFooterItems, setNavFooterItems] = useState([
      { id: 1 , idnm : "Home", navheading: "Home", icon: 'fas fa-home'  },
      { id: 2 , idnm : "Comments", navheading: "Comments", icon: 'fas fa-comment' },
      { id: 3 , idnm : "Video", navheading: "Video", icon: 'fas fa-video' },
...
// console.log('storeUser',storeUser)
 ...
 const calendar = useRef(false);
  ...
  const eventDetail = {
    "title": meetingDetail && meetingDetail.bill_id ? 'Discuss Bill '+meetingDetail.bill_id : "Discuss Upcoming Bill",
    "createDate": meetingDetail && meetingDetail.publishedAt ? meetingDetail.publishedAt : "2020-06-01",
    "status": "Introduced",
    "bill_id": meetingDetail && meetingDetail.bill_id ? meetingDetail.bill_id : "H.R. 7994",
    "labelColor": "#4F6577",
    "label": "Townhall",
    "check":true,
    "eventCalendar" :{
        ...
        "location": 'Zoom',
        "startTime": meetingDetail && meetingDetail.publishedAt ? meetingDetail.publishedAt : '2016-09-16T20:15:00-04:00',
        "endTime": ''
    }
  }
  ...
  // useEffect(() => {
    // if(meetingKey && meetingDetail && meetingDetail.meeting_key == meetingKey){
      ...
      myMeetngStatus.current = meetingKey && meetingDetail && meetingDetail.meeting_key == meetingKey
  ...
  const logout = () => {
    ...
    setShowLogin(false)
      if(storeUser && storeUser.meeting_key){
        updateMeetingStatus({
          status:false,
          meeting_id:storeUser && storeUser.meeting_id ? storeUser.meeting_id : '',
    ...
    const [url, setImageURL] = useState('https://meeting.varehall.com/upload/myUpload');
    ...
    const onImage = async (failedImages, successImages) => {
      ...
      try {
        // console.log('successImages',successImages)
          ...
          const data = parts[2];
          const res = await Axios.post(url, { mime, name, image: successImages[0] })
          ...
          ? navFooterItems.map((res,index) => {
            ...
            style={{
              color: activeFooterNav == res.idnm ? 'red' : '#cfcfc4'
               ...
               className={`tab-link ${activeFooterNav == res.idnm ? 'tab-link-active' : null}`}>
                <i onClick={()=>{
                  // if(res.idnm == 'Video'){
                  //   // videoTag.current.scrollIntoView({ behavior: 'smooth' })
                  // } else if(res.idnm == 'Meeting'){
                  //   meetingTag.current.scrollIntoView({ behavior: 'smooth' })
                  // } else if(res.idnm == 'Poll'){
                  //   pollTag.current.scrollIntoView({ behavior: 'smooth' })
                  // } else if(res.idnm == 'Home'){
                  //     homeTag.current.scrollIntoView({ behavior: 'smooth' })
                  // } else if(res.idnm == 'Login'){
                  //     loginTag.current.scrollIntoView({ behavior: 'smooth' })
                  // }


                    setActiveFooterNav(res.idnm)
                    setActiveNav(res.idnm == 'Home'
                    ? 'VareHall'
                    : res.idnm == 'Comments'
                    ? 'Comments'
                    : res.idnm == 'Video'
                ...
                className={`fas ${res.icon}`}></i>
                <span
                className="tabbar-label">{res.navheading}</span>
              </span>
          }): null}
                ...
                {navItems && navItems.length && navItems.length>0
                  ? navItems.map((res,index) => {
                    return <span
                      key={'sjdh'+index}
                      onClick={()=> {
                        if(res.idnm == 'Comments' || res.idnm == 'VareHall'){
      ...
      <PageFooter />
          <Container>
              <Row className="mt-4">
                 ...
                 {activeNav == 'Comments'
                 ? <Col lg={12} sm={12}>
                     <Col className={ "commenstField mb-3"} lg={6} sm={6} >
                     <CommentsMobile
                      meetingComments={meetingComments}
                      meetingRSVP={meetingRSVP}
                      meetingDetails={meetingDetails}
                      host={host}
                      user={storeUser && storeUser.name
                          ? storeUser
                          : logedUser && logedUser.name
                          ? logedUser
                          : user}
                ...
                >
                 ...
                 <video
                 // height={isMobile ? '100%' : 220}
                 autoPlay loop="loop" muted="muted" volume="0">
                  <source src={require("../../../../assets/images/video2.mp4")} type="video/mp4" />
                 ...
                 <div style={{
                   ...
                   backgroundColor:'rgba(0,0,0,.79)'
                     ...
                     <p>Hello,</p>
                    {meetingDetail && meetingDetail.description ? <p>{meetingDetail.description}</p> : null}
                     <p>You have been invited to a Townhall/VareHall meeting.  This Varehall meeting is your opportunity to influence your Representative by taking a stance on their actions or inactions concerning legislations that affect you.</p>
                     <p></p>
                  </div>
                  </Card>
                </div>
              ...
              <div style={{
                ...
                ref={loginTag}
                onClick={()=>{
                  if((storeUser && storeUser.name) || (logedUser && logedUser.name)){
                    setShowLogin(false)
                    logout()
                  } else {
                    setShowLogin(true)
                ...
                {((storeUser && storeUser.name) || (logedUser && logedUser.name))
                    ? 'Logout'
                    :'Login'}
              </div>
              </div>
                ...
                <div
                  ...
                  onClick={()=>{
                    if(myMeetngRef
                      ...
                      && myMeetngRef.current
                      && myMeetngRef.current.value.toLowerCase() == meetingDetail.meeting_key.toLowerCase())
                      {
                      setLogedUser({
                        name:meetingDetail.author,
                        email:meetingDetail.member_id,
                    ...
                    } else if(myNameRef && myNameRef.current && myNameRef.current.value != ''
                  && myEmailRef && myEmailRef.current && myEmailRef.current.value != ''){
                    setLogedUser({
                      name:myNameRef && myNameRef.current && myNameRef.current.value,
                      email:myEmailRef && myEmailRef.current && myEmailRef.current.value,
                      user_id:myEmailRef && myEmailRef.current && myEmailRef.current.value,
                      member_id:meetingDetail && meetingDetail.member_id
                    })
                    setStoreUser({
                      name:myNameRef && myNameRef.current && myNameRef.current.value,
          ...
          : null}
                  ...
                  <div style={{
                    ...
                    <Fragment>
                        <CalendarButton
                          ...
                          handleShowCalendars={()=>{
                            if(meetingDetail && meetingDetail != null){
                              ...
                              alert('Invalid meeting link.  Please contact the meeting organizer for the correct link.')
                            }
                          }}
                        />
                          ...
                          <div style={{
                            ...
                            // width:'80%'
                             ...
                             <div className="button" onClick={()=>{
                               ...
                               const detail =
                               `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${'Bithday'}&dates=${meetingDetail && meetingDetail != null && meetingDetail.publishedAt ? meetingDetail.publishedAt.replace(/[&-\/\\#,+()$~%. '":*?<>{}]/g,'')+'/20201231T223000Z' : '20220120T135850250Z/20201231T223000Z'}&details=${meetingDetail && meetingDetail != null ? meetingDetail.description.toString() : 'Meeting Invitation'}&location=${window.location}`
                                window.open(detail)
                             ...
                             <div style={{marginLeft:10}} className="button" onClick={()=>{
                               const detail =
                               `http://calendar.live.com/calendar/calendar.aspx?rru=addevent&dtstart=${meetingDetail && meetingDetail != null && meetingDetail.publishedAt ? meetingDetail.publishedAt : ''}&dtend=&summary=${meetingDetail && meetingDetail != null ? meetingDetail.description.toString() : 'Meeting Invitation'}&location=${window.location}`
                               window.open(detail)
                               && setShowCalendar(false)
                      ...
                      <div style={{width:'100%'}} ref={pollTag}>
                        <Card className="card">
                          <div className="">
                            <CardBody className="align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                              ...
                              >
                                <span style={{color:'red'}} className="align-middle d-inline-block">
