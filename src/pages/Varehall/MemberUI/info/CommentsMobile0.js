...
const CommentList = ({
  ...
  useEffect(() => {
    ...
    const orderedComments =
      ...
      meetingComments.sort(function(a, b) {
        return Date(a.date) > Date(b.date) ? 1 : -1;
      });

    const myUpdates =
      meetingCommentUpdate &&
      meetingCommentUpdate.length &&
      meetingCommentUpdate.length > 0 &&
      meetingCommentUpdate &&
      orderedComments &&
      orderedComments.length &&
      orderedComments.length > 0
        ? orderedComments.filter(res => {
            return (
              (res.meeting_id == meetingCommentUpdate[0].meeting_id &&
                res.date == meetingCommentUpdate[0].date) == false
            );
          })
        : "";

    if (myUpdates) {
      setMeetingState([...meetingCommentUpdate, ...myUpdates]);
    } else {
      setMeetingState(orderedComments);
    }
  }, [meetingComments, meetingCommentUpdate]);
