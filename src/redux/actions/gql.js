...
export async function getAmbassadors({offset}) {
  let query = `{
    vareAmbassadors(offset:${offset}) {
     length
     cursor
     hasMore
     vareAmbassadors {
  ...
  const result = reply
  && reply.vareAmbassadors
  && reply.vareAmbassadors.vareAmbassadors
...
export async function getPolicy({offset,search}) {
  const offetVal = offset ? offset : 0
  const searchText = search ? search : ''
  let query = `{
    vareCongressBills(offset:${offetVal},search:"${searchText}") {
      keywords
      length
      cursor
      hasMore
...
export async function getMyStateRepresentatives({offset,address}) {
  let query = `{
    vareLocalReps(address:"${address}",offset:0) {
      length
      cursor
      hasMore
      vareLocalReps {
  ...
  const result = reply
  && reply.vareLocalReps
  && reply.vareLocalReps.vareLocalReps
  // console.log('xxvareLocalReps',reply)
  return result
}


export async function getTexasStateRepresentatives({offset,address}) {
  let query = `{
    vareTexasStateReps(address:"${address}",offset:0) {
        length
        cursor
        hasMore
        vareTexasStateReps {
  ...
  let reply = await myQuery(query);
  const result = reply
  && reply.vareTexasStateReps
  && reply.vareTexasStateReps.vareTexasStateReps
...
export async function getMembersByState({offset,address}) {
  let query = `{
    vareMembersByState(address:"${address}",offset:${0}) {
      length
      cursor
      hasMore
      vareMembersByState {
...
export async function getMembers({offset,type,search,state}) { //alert(3)
  const offetVal = offset ? offset : 0
  const searchText = search ? search : ''
  ...
  const result = reply
  && reply.vareMembers
  && reply.vareMembers.vareMembers

  const keywords = reply
  && reply.vareMembers
  && reply.vareMembers.keywords

  const offest = reply
  && reply.vareMembers
  && reply.vareMembers.offest
...
export async function getLocalCandidates({offset,search,state}) {
  ...
  const result = reply
  && reply.vareCandidates
  && reply.vareCandidates.vareCandidates
  // console.log('vareCandidates',result)
  const keywords = reply
  && reply.vareCandidates
  && reply.vareCandidates.keywords
...
export async function getMemberVoteRecord({id,offset,search}){
  ...
  const result = reply
  && reply.vareMemberVotes
  && reply.vareMemberVotes.vareMemberVotes

  const bio = reply
  && reply.vareMemberVotes
  && reply.vareMemberVotes.bio
...
export async function getVareUserVotes (offset){
  ...
  const result = reply
  && reply.vareUserVotes
  && reply.vareUserVotes.vareUserVotes
...
export async function getMyCandidates() {
    ...
    export async function getNewContestants(executiveCandidate){
      ...
      const result = reply
      && reply.vareContestants
      && reply.vareContestants.vareContestants
...
export async function getElections() {
   ...
   const url ="https://www.googleapis.com/civicinfo/v2/elections?key=AIzaSyACYhltJ6rt1RRKcrgCKCobmKrLE-qT5L4";
...
//Todo  filter records and add to the database and return to parent component to save to store
   const pollingData = await getPollingLocation(result.elections)
     .then(rep => {
      const details = rep.map(results => {
        // console.log('resultsxx',results)
           ...
           : []
           const contests = results.contests
           ? results.contests
           : []
           const name = results.election
            && results.election.name
            ? results.election.name
            : ''
           const electioId = results.election
             && results.election.id
             ? results.election.id
             : ''
           const electionDetails = results.state.map(res => {
               let electionObject = {}
                electionObject['heldBy'] = res.electionAdministrationBody
                && res.electionAdministrationBody.name
                ? res.electionAdministrationBody.name
                : ''
                electionObject['county'] = res.local_jurisdiction
                && res.local_jurisdiction.name
                ? res.local_jurisdiction.name
                : ''

                const electionOfficials = res.local_jurisdiction
                ...
                electionObject['emailAddress'] = electionOfficials
                && electionOfficials.emailAddress
                ? electionOfficials.emailAddress
                : ''

                electionObject['officePhoneNumber'] = electionOfficials
                && electionOfficials.officePhoneNumber
                ? electionOfficials.officePhoneNumber
                : ''


                const physicalAddress = res.local_jurisdiction
                && res.local_jurisdiction.electionAdministrationBody
                && res.local_jurisdiction.electionAdministrationBody.physicalAddress
                ? res.local_jurisdiction.electionAdministrationBody.physicalAddress
                : ''
                electionObject['address'] = physicalAddress.line1 + ' ' + physicalAddress.city+ ' ' + physicalAddress.state+ ' ' + physicalAddress.zip
   ...
   //  }
     ...
     const electInfo = result.elections && result.elections.length>0  //if api data is empty then resent the stored data
     ? result.elections
     : [];

    //mutate elections data to add polling array from promise all
    pollingData && electInfo.map(res => {
         ...
         res['electionDetails']=polls && polls[0] ? polls[0].electionDetails : ''
         res['pollingLocations']=polls && polls[0] ? polls[0].pollingLocations : ''
          return res
     })
...
export async function getCommittees ({congress,chamber}) {
  const policyurl = `https://api.propublica.org/congress/v1/${congress ? congress : 115}/${chamber ? chamber : 'senate'}/committees.json`
...
export async function getstateElectionOffice({page,myState,rep}) {
  ...
  if(myState && myState !== ''){
   ...
   const allElections = await Promise.all(data.results.map(result => {
      return result
    })
   )

   const info = allElections
   ...
   && allElections.map(location => {
     // console.log('location',location)
      const result =   {
          ...
          officePhoneNumber: location.primary_phone_number ? location.primary_phone_number : '',
        }
        return result;
      })
...
export async function getNews() {
  const newsurl ="https://newsapi.org/v2/top-headlines?country=us&apiKey=904fe7cb2cd4412593c3326ccc240c04";
...
const billsOption = (query) => {
  // console.log('bills',query)
    ...
    const billURL = fullUrl[query.requestType] ? fullUrl[query.requestType] : fullUrl.recentBills
    return billURL;
}
...
export async function getCandidates() {
  ...
  const allResult = await RESTCall.axiosQuery(formData)
     .then((results, index) => {
         return results && Promise.all(results.map(result => {
           let rep = {
         ...
         // console.log('voteDataxx0',reports)
          ...
          const result = reports
              ...
              && reports.data.map(res => {
                 ...
                 const voteInfo = storedBills && storedBills.filter(rep => {
                 ...
                 const billDetails = {
                    ...
                    description: voteInfo.description ? voteInfo.description : voteInfo.title,
  ...
  // console.log('sssssmyVote',myVote)
    // const myVote = await AppStore.get('myVote')
     ...
     .then(rep => {
         ...
         const myVoteCount = rep
            && rep.data
            && rep.data.length > 0
            && rep.data.map(info => {
              const myObject = {}
              const infoYes = info.votes && info.votes.no && info.votes.no>=0 ? info.votes.no*1 : 0
              const infoNo = info.votes && info.votes.yes && info.votes.yes>=0 ? info.votes.yes*1 : 0
              const calc = infoYes > 0
        ...
        const myVoteCount = []
         ...
         && Object.keys(myVote).map(info => {
           ...
           if(countYes.length>0 || countNo.length>0){
             const calc = countYes.length>0
             ? (countYes.length/(countYes.length + countNo.length))*100
             : countNo.length>0
             ? 100-(countNo.length/(countYes.length + countNo.length))*100
             :  0
     ...
     const reportData = await RESTCall.axiosQuery(formData)
          .then(reports => {
            // console.log('reports',reports)
             const newReport = reports.data
             && reports.data.length > 0
             ? reports.data.map(res => {
               const profileState = res.address
                &&  res.address.split(',')
                &&  res.address.split(',')[2]
...
// todo -used in Ligislation component and others.  Refactor to call function rom here
   export async function databaseSaveVote(itemInfo,myVote) {
      ...
      if(myInfo && myInfo.address && myInfo.email){
          ...
          this.props.navigation.navigate('ProfileScreen', {'profile': 'Contact Information'})
       }
   ...
   export async function getLocalCandidatesByElection({page,myState}) {
     ...
     const info = contestants.results
     ...
     && contestants.results.map(contestant => {
        // console.log('contestant',contestant)
         const result =   {
              ...
              challenger: contestant.incumbent_challenge_full ? contestant.incumbent_challenge_full : '',
              candidate_id:contestant.candidate_id ? contestant.candidate_id : '',
              state: contestant.state,
              active: contestant.candidate_inactive == true
              ? false
   ...
   export async function getMandates() {
      ...
      && allResult.data.length>0
      && Promise.all(allResult.data.map(rep => {
           const formData = {
             request:'search',
   ...
   export async function getMandatesAuthor(rep,formData) {
      ...
      // console.log('tempBillmemberInfo',memberInfo)
       rep['name'] = memberInfo.fullName
       ? memberInfo.fullName
       : 'Private'

       rep['bill_id'] = memberInfo.id
       ? memberInfo.id
       : 'na'
  ...
  export async function getSateFromProfile(){
     ...
     const myTempProfile = myProfile.length > 0
     || Object.keys(myProfile).length > 0
     ? myProfile
     : ''

     const govLevel = myTempProfile
     && myTempProfile['Government Level']
...
export async function getCongressFromDB(allcongress){
   ...
   const records = allcongress
   && allcongress.length > 0
   ? allcongress.map((rep, index) => {
      if(existingDataKeys && existingDataKeys.includes(rep.member_id)==false){
         const formData = {
            request:'insert',
            query: rep,
            resource: 'vare_congress'
